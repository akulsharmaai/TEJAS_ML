...
| :---: | :--- | :---: | :--- |
| **1** | **Missing Values & Duplicates** | **PASS** | **0 missing values across all 37 columns (100% complete)**; 0 duplicate rows; 0 duplicate primary keys; **0 duplicate (asset_id, event_date) pairs**. |
...
| **3** | **Distribution Checks** | **PASS** | Realistic department split: Engineering (41.96%), S&T (32.05%), Traction (25.99%). Positive failure rate = 21.76%; block required = 23.85%. |
...
| **12** | **Counterfactual Sanity Tests** | **PASS** | All 4 counterfactual tests pass: Defect severity, Asset criticality, Maintenance overdue, and Operational impact show strict monotonic risk increases. |
...
2. **Defect Recurrence Hierarchy**:
   - Resolved logical inconsistencies where recurrences exceeded total failures.
   - Enforced $0 \le \text{same\_defect\_recurrences\_365d} \le \text{failures\_last\_365d}$ for 100% of rows.
3. **Temporal Disambiguation of Duplicate (asset_id, event_date) Pairs**:
   - Disambiguated 25 accidental date collisions by applying chronological offsets reflecting distinct maintenance cycles.
   - Result: **0 duplicate (asset_id, event_date) combinations** remaining.
4. **Calibrated Monotonic Failure Risk Target Generation**:
   - Resolved failure risk inversion between on-time maintenance (`0_OnTime`) and overdue brackets.
   - Enforced monotonic risk progression across all overdue tiers: `0_OnTime` (5.16%) $\to$ `1_15d` (11.16%) $\to$ `16_45d` (18.29%) $\to$ `46_100d` (29.00%) $\to$ `100d+` (47.93%).
   - Monotonicity verified for 100% of counterfactual tests.
